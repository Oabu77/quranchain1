...
const commands = [
  new SlashCommandBuilder().setName("law-agents").setDescription("All 11 DarLaw legal AI agents"),
  new SlashCommandBuilder().setName("law-ip").setDescription("IP portfolio — trademarks, patents, copyrights, trade secrets"),
  new SlashCommandBuilder().setName("law-filings").setDescription("Legal entity filings across jurisdictions"),
  new SlashCommandBuilder().setName("law-contracts").setDescription("Active contract registry with fees"),
  new SlashCommandBuilder().setName("law-trademark").setDescription("Trademark Agent — USPTO filing & monitoring"),
  new SlashCommandBuilder().setName("law-patent").setDescription("Patent Agent — prior art search & filing"),
  new SlashCommandBuilder().setName("law-compliance").setDescription("Compliance & regulatory monitoring (50 jurisdictions)"),
  new SlashCommandBuilder().setName("law-shariah").setDescription("Sharia Law Advisor — Islamic jurisprudence"),
  new SlashCommandBuilder().setName("law-help").setDescription("DarLaw bot commands"),
...
(async () => {
  console.log(`Registering ${commands.length} DarLaw commands...`);
  if (GUILD_ID) {
