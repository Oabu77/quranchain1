...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Temporary containment: the legacy checkout can change entitlement state
    // before a verified Stripe payment event has established payment success.
    ...
    // Stripe customer_id without binding it to the authenticated local account.
    ...
    // The legacy admin endpoint accepts an ordinary authenticated user and
    // returns recent-user/business metadata. Fail closed until a distinct,
    // server-issued administrator authorization mechanism exists.
